...
from Anon import cmds, BOTLOG_CHATID, LOGGER
...
def restart():
    os.execvp(sys.executable, [sys.executable, "-m", "Anon"])
...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Anon-Userbot Deploy Update is in Progress...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`Anon-Userbot Successfully Updated! Userbot can be used again.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Anon-Pyro** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**Anon-Pyro Berhasil di matikan!**")
    if HAPP is not None:
...
add_command_help(
    "update",
    [
        [f"{cmds}update", "Cek update dari Anon-Pyro Userbot."],
        [f"{cmds}update deploy", "To update Anon-Pyro Userbot."],
    ],
)
